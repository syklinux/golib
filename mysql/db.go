...
func InitDb(conf *MySQLConf, isAutoMigrate bool, datas ...interface{}) {
	db := newDB()
...
func (db *DB) InitDataBase(conf *MySQLConf, datas ...interface{}) {
	var (
		err error
	)
	cfg := &gorm.Config{}
	...
	for index, host := range readHosts {
		dials[index] = mysql.Open(db.getReadDsn(conf.DataBase, host, conf.User, conf.Password))
	}
	...
	if err := Db.Use(dbresolver.Register(dbresolver.Config{
		Sources:  []gorm.Dialector{mysql.Open(db.getWriteDsn(conf.DataBase, conf.WriteServer, conf.User, conf.Password))},
		Replicas: dials,
